...
export const fetcher = async (url: string, method: METHOD, body?: Record<string, unknown> | FormData, headers?: HeadersInit) => {

  let parsedUri = `${process.env.BASE_API_URL ?? ''}${url}${method === METHOD.GET && body ? `?${new URLSearchParams(body as unknown as Record<string, string>)}` : ''
    }`;
